...
function changes(old, manifest, policyBytes) {
  const before = new Map((old.manifest?.files ?? []).map(f => [f.destination, [f.sha256, f.gitMode]]));
  const after = new Map(manifest.files.map(f => [f.destination, [f.sha256, f.gitMode]]));
  if (old.manifestBytes) { before.set(MANIFEST, [sha256(old.manifestBytes)]); before.set(POLICY, [sha256(old.policyBytes)]); }
  after.set(MANIFEST, [sha256(jsonBytes(manifest))]); after.set(POLICY, [sha256(policyBytes)]);
  return { create: [...after.keys()].filter(k => !before.has(k)).sort(), update: [...after.keys()].filter(k => before.has(k) && JSON.stringify(before.get(k)) !== JSON.stringify(after.get(k))).sort(), delete: [...before.keys()].filter(k => !after.has(k)).sort() };
}
...
export function run({ repo, commit, policy, out, materialize = false }) {
  ...
  try {
    ...
    if (!materialize) return { applied: false, changes: delta, mirror: manifest.mirror };
    ...
    return { applied: true, changes: delta, mirror: manifest.mirror };
